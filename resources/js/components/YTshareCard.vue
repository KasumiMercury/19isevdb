    ...
    <div class="p-2 shadow-md shadow-zinc-900 rounded-md flex flex-col">
        <div v-if="status == 1">
            <div class="flex flex-row pb-3 lg:pb-0 lg:h-20 bg-stone-800">
                <div @click="jump" class="grow cursor-pointer text-white p-1 overflow-hidden">
                    <p class="text-xxs lg:text-sm max-h-full textOmit">{{ title }}</p>
                </div>
            </div>
            <div class="h-px bg-gradient-to-r from-stone-900 via-green-500 to-stone-900"></div>
        </div>
        <div v-else-if="status == 2">
            <div class="flex flex-row pb-3 lg:pb-0 lg:h-20 bg-stone-800">
                <div @click="jump" class="grow cursor-pointer text-gray-50 p-1 overflow-hidden">
                    <p class="text-xxs lg:text-sm max-h-full textOmit">{{ title }}</p>
                </div>
            </div>
            <div class="h-px bg-gradient-to-r from-stone-900 via-zinc-900 to-stone-900"></div>
        </div>
        <div v-else>
            <div class="flex flex-row pb-3 lg:pb-0 lg:h-20 bg-stone-800">
                <div @click="jump" class="grow cursor-pointer text-white p-1 overflow-hidden">
            ...
            <div class="h-px bg-gradient-to-r from-stone-900 via-rose-500 to-stone-900"></div>
        </div>

        <div @click="jump" class="cursor-pointer flex flex-row mb-0 mt-1">
...
<script>
import { div } from "@inertiajs/inertia-vue3"
...
export default {
    ...
    props: ["title", "id", "date", "status", "url"],
}
</script>
<style>
.textOmit {
